---
name: weekly-review
description: "面向 WorkBuddy 用户的周度复盘 skill：读取 WorkBuddy 本地会话库（workbuddy.db），基于会话时长、活跃度、自动化运行等客观数据，自动产出一页看板、分项目分析、根因三类归因（思路/记忆/流程）、动作台账、待对齐开放会话。固化复盘底座，不固化每周成品。"
version: 1.0.0
category: 办公效率
read_when:
...
一个面向 WorkBuddy 用户的周度复盘 skill。它把"复盘该看什么、怎么归因、怎么落台账"固化成可复用的底座；每周具体项目、人工改进项由使用者或运行时输入。
...
1. 确保本仓库已安装：
   ...
   pip install -e .
   ```
2. 将本 `SKILL.md` 复制到 WorkBuddy 的 skills 目录：`~/.workbuddy/skills/weekly-review/SKILL.md`。
...
在 WorkBuddy 的 MCP 配置中添加（`~/.workbuddy/mcp.json`）：
...
- 默认读取 WorkBuddy 会话库 `~/.workbuddy/workbuddy.db`；可用 `--db-path` 指定其他会话库，用 `--notes` 传入自定义复盘笔记 JSON。
